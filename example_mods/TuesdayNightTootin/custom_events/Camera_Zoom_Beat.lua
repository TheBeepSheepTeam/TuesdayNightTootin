...
function onEvent(n,v,w)
    if n=='Camera Zoom Beat'or n=='Camera_Zoom_Beat'then--underscores bcz of how discord names filenames with spaces :skull:
        camZooming=w~=''zoomStp=tonumber(v)mult=(w==''and 1 or tonumber(w))
    end
    ...
    if n=='Camera Zoom Interval'then--Andromeda Engine compatibility
        camZooming=w~=''zoomStp=tonumber(v)mult=(w==''and 0.02/0.015 or tonumber(w/0.015))
    end
end
function onStepHit()
    if (getProperty('camZooming')and camZooming)and getPropertyFromClass('ClientPrefs','camZooms') then
